...
export default class LCSDiffEngine {
  ...
  /**
   * gets the stable base key from the standardized item.
  ...
  runDiff(oldArr, newArr) {
    ...
    const wrappedNew = this.#persistKeys(newArr);
    ...
    for (const nw of wrappedNew) {
      ...
      if (!oldEntry) {
        ...
        // NOTE: we can limit patches by check if the items's position(idx) has changed. 
        // and compare based on value instead of reference that would limit patches.
